...
func applyFn(ctx context.Context) error {
	...
	timeout := getTimeOut(data)

	// Copy and execute each script
	if err := runScripts(ctx, o, comm, scripts, timeout); err != nil {
		return err
	}
...
func runScripts(
	...
	scripts []io.ReadCloser, timeout int) error {
	...
	for _, script := range scripts {
		...
		if err := comm.UploadScript(remotePath, script, timeout); err != nil {
		...
		if err := comm.Start(cmd, timeout); err != nil {
